...
type GroupClaudeSettings struct {
	...
	// Account names a [claude.accounts.<name>] entry whose env_file is
	// sourced for sessions in this group. Per-session ClaudeOptions.Account
	// takes precedence; this group value beats [claude].default_account.
...
// GetGroupClaudeAccount returns the group-specific Claude account name, if
// configured. Empty means the group has no opinion — caller falls through to
// [claude].default_account.
func (c *UserConfig) GetGroupClaudeAccount(groupPath string) string {
	if c == nil || groupPath == "" || c.Groups == nil {
		return ""
	}
	groupCfg, ok := c.Groups[groupPath]
	...
	return groupCfg.Claude.Account
}
